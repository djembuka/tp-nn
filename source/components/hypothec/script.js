  ...
  $(function () {
    $('.b-calculator-form .b-form-field').each(function () {
      ...
      var slider = $slider.slider({
        min: $slider.data('min'),
        max: $slider.data('max'),
        step: $slider.attr('id') === 'percentSlider' ? 0.5 : 1,
        range: 'min',
        value: $input.val(),
        slide: function (event, ui) {
          ...
          if (
            ...
            $('#firstSlider').slider('value')
          ) {
            $('#priceSlider').slider({
              value: $('#firstSlider').slider('value'),
            });
            $('#priceInput').val(
              makeNumSpaces($('#priceSlider').slider('value'))
            );
          }
    ...
    function calculateResult() {
      ...
      var month_payment = Math.round(
        (credit * month_perc) / (1 - Math.pow(1 + month_perc, -months))
      );
    ...
    function makeNumSpaces(num) {
      ...
      return n[1] === undefined ? num : num + '.' + n[1];
    }
